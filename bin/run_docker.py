...
def check_output_file_size(
    ...
    if ".zip" in os.path.basename(file):
        with ZipFile(file, 'r') as zipfile:
            incorrect_size = any(zip_info.file_size == 0 for zip_info in zipfile.infolist() if not zip_info.is_dir())
    ...
    return log_text, bad_output_msg


def handle_outputs(output_path: str, output_file_name: str, log_text: str) -> Tuple[str, str]:
    """
    ...
    # Then check if the output file is empty
        ...
        log_text, bad_output_msg = check_output_file_size(file_glob, log_text, bad_output_msg)
