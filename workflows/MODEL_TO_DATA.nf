...
workflow MODEL_TO_DATA {
    ...
    UPDATE_FOLDERS(submission_ch, params.project_name, run_docker_outputs.output.map { it[1] }, run_docker_outputs.output.map { it[2] })
    ANNOTATE_SUBMISSION_AFTER_UPDATE_FOLDERS(UPDATE_FOLDERS.output)
    VALIDATE(run_docker_outputs.output, SYNAPSE_STAGE_GOLDSTANDARD.output, UPDATE_SUBMISSION_STATUS_AFTER_RUN.output, params.execute_validation)
